...
def plot_chart_tensorboard(chart_type, path_to_png, path_to_log_list):
    for path_to_log in path_to_log_list:
        os.system('%s %s' % (get_log_parsing_script(), path_to_log))
        data_file = get_data_file(chart_type, path_to_log)
        ...
        for i in range(len(data[0])):
            log_value(y_axis_field, data[1][i], int(data[0][i]))
...
Usage:
    ./plot_training_log.py /path/to/first.log ...
Notes:
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        for path_to_log in path_to_logs:
            ...
            tf_runs_path = 'runs/' + tmp[0]
            configure(tf_runs_path, 0.1)
            for ch in chart_type:
                plot_chart_tensorboard(ch, [], path_to_logs)
                #plot_chart(ch, path_to_png, path_to_logs)
    print('tensorboard --logdir runs --port 8080')
    os.system('tensorboard --logdir runs --port 8080')
    print('Done')