  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    while (!valid_sample) {
      ...
      this->transformed_data_.set_cpu_data(prefetch_data + offset);
        // Apply face transformations(Scale down and up)
        bool face_transform = this->layer_param_.image_data_param().face_transform();
        if(face_transform){
            Dtype min_random_scale = this->layer_param_.image_data_param().min_random_scale();
            Dtype max_random_scale = this->layer_param_.image_data_param().max_random_scale();
            this->data_transformer_->Transform(cv_img, &(this->transformed_data_), face_transform,
